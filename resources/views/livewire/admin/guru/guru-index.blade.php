<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="font-heading text-2xl font-bold text-gray-900">Data Guru</h1>
            <p class="text-gray-500">Kelola data ustadz dan guru</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    <a href="{{ route('admin.guru.template') }}" class="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-gray-50">
                    ...
                    <a href="{{ route('admin.guru.export') }}" class="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 border-t border-gray-100">
            ...
            <div class="md:col-span-2">
                <div class="relative">
                    ...
                    <input type="text" 
                           ...
                           placeholder="Cari nama, NIP, atau bidang keahlian..."
                ...
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        <th class="px-6 py-3 text-left">
                            <button wire:click="sortBy('nip')" class="flex items-center text-xs font-semibold text-gray-500 uppercase tracking-wider hover:text-gray-700">
                                NIP
                                @if($sortField === 'nip')
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Bidang Keahlian</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($gurus as $guru)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 text-sm text-gray-900 font-medium">{{ $guru->nip ?? '-' }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-gray-900">{{ $guru->nama_lengkap }}</p>
                                        <p class="text-xs text-gray-500">{{ $guru->user->email ?? '-' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $guru->jabatan ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $guru->bidang_keahlian ?? '-' }}</td>
                            <td class="px-6 py-4">
                                <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full 
                                    {{ $guru->status === 'aktif' ? 'bg-green-100 text-green-700' : '' }}
                                    {{ $guru->status === 'nonaktif' ? 'bg-gray-100 text-gray-700' : '' }}
                                    {{ $guru->status === 'pensiun' ? 'bg-blue-100 text-blue-700' : '' }}">
                                    {{ ucfirst($guru->status) }}
                                </span>
                            </td>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 class="text-lg leading-6 font-medium text-gray-900">Hapus Data Guru</h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">Apakah Anda yakin? Tindakan ini tidak dapat dibatalkan.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button wire:click="deleteGuru" type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 sm:ml-3 sm:w-auto sm:text-sm">
                            Hapus
                        </button>

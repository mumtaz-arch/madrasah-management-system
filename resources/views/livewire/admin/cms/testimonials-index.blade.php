<div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="font-heading text-2xl font-bold text-gray-900">Testimoni</h1>
            <p class="text-gray-500">Kelola testimoni untuk landing page</p>
        </div>
        ...
        <table class="w-full">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Testimoni</th>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($testimonials as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $item->peran }}</td>
                ...
                @empty
                    <tr><td colspan="6" class="px-6 py-12 text-center text-gray-500">Belum ada testimoni</td></tr>
                @endforelse
            </tbody>
        </table>
        @if($testimonials->hasPages())<div class="px-6 py-4 border-t">{{ $testimonials->links('vendor.pagination.tailwind') }}</div>@endif
    </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white rounded-lg shadow-xl max-w-lg w-full">
                    <form wire:submit="save">
                        <div class="px-6 py-4 border-b"><h3 class="text-lg font-semibold">{{ $editingId ? 'Edit' : 'Tambah' }} Testimoni</h3></div>
                        <div class="p-6 space-y-4">
                            <div><label class="block text-sm font-medium text-gray-700 mb-1">Nama</label><input type="text" wire:model="nama" class="w-full px-4 py-2 border border-gray-300 rounded-lg">@error('nama')<span class="text-red-500 text-xs">{{ $message }}</span>@enderror</div>
                            <div><label class="block text-sm font-medium text-gray-700 mb-1">Peran</label><input type="text" wire:model="peran" placeholder="Alumni, Wali Santri, dll" class="w-full px-4 py-2 border border-gray-300 rounded-lg">@error('peran')<span class="text-red-500 text-xs">{{ $message }}</span>@enderror</div>
                            <div><label class="block text-sm font-medium text-gray-700 mb-1">Testimoni</label><textarea wire:model="isi" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg"></textarea>@error('isi')<span class="text-red-500 text-xs">{{ $message }}</span>@enderror</div>
